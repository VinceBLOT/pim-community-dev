...
define(
    ...
    function (_, Backbone, BaseForm, FetcherRegistry, ProductManager, modalTemplate, UserContext, i18n) {
        var FormView = BaseForm.extend({
            ...
            showModal: function () {
                ...
                familyModal.open();
                familyModal.$('.family-select2').select2('destroy').select2({
                    ajax: {
                        ...
                        results: function(families) {
                            ...
                            _.each(families, function (value, key) {
                                data.results.push({id: key, text: i18n.getLabel(value.label, UserContext.get('catalogLocale'), value.code)});
                            });
                    ...
                    initSelection: function (element, callback) {
                        ...
                        if ('' !== code) {
                            FetcherRegistry.getFetcher('family').fetch(this.getFormData().family).then(function (family) {
                                callback({id: family.code, text: i18n.getLabel(family.label, UserContext.get('catalogLocale'), family.code)})
                            });
                        }
                    }
                });
